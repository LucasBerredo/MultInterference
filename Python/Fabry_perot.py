...
def fabry_perot(lam0: np.float64, n_High: np.float64, n_Low: np.float64, n_cavity: np.float64, n_inc: np.float64, n_sub: np.float64, m: int, cav_mult: float, theta_i0s: np.float64, mesh_len: int, plot: bool):
    # Mirror 1
    ...
    mesh = np.linspace(0.7 * lam0, 1.3 * lam0, mesh_len)
...
if __name__ == "__main__":
    ...
    fabry_perot(lam0, n_High, n_Low, n_cavity, n_inc, n_sub, m, cav_mult, theta_i0, mesh_len, True)