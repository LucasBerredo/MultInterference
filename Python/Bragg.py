...
# Main code
def bragg(lam: np.float64, n_High: np.float64, n_Low: np.float64, n_inc: np.float64, n_sub: np.float64, m: int, theta_i0s: npt.NDArray[np.float64], mesh_len: int, plot: bool):
    ## Setup
    ...
    angles = np.atleast_1d(theta_i0s)
    plt.figure(figsize=(10, 6))

    for theta in angles:
        _,_,_,_,_,r,_ = MultInterference(Hi=1, theta_i0=theta, n=n, d=d, lam=mesh, ind=False)
        R = np.abs(r)**2
        ...
        return mesh, R, R_max, lam_BW_min, lam_BW_max
...
if __name__ == "__main__":
    ...
    bragg(lam, n_High, n_Low, n_inc, n_sub, m, theta_i0, mesh_len, True)